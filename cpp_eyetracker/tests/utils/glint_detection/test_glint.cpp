...
int main() {
    ...
    if (use_record) {
        ...
        for (const auto& dir : valid_record_dirs) {
            processRecordDirectory(dir, param_type, max_images, is_collecting);
        }

        Logger::info() << "All record directories have been processed successfully.";
        return 0;
    }
