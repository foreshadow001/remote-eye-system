...
struct CameraContext {
    ...
    // ---- Shared memory (multi-process HDF5 dump) ----
    HANDLE shm_handle = NULL;   // CreateFileMapping handle
    uint8_t* shm_base = nullptr; // MapViewOfFile base address
...
void dumpToHdf5Worker(shared_ptr<CameraContext> ctx, int core_frames, int margin_frames) {
    ...
    try {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            raw_ds.write(ctx->ram_buffer[src_idx].data, H5::PredType::NATIVE_UINT8, f_mem, f_file);
        }
...
int main() {
    ...
    for (auto& ctx : cam_ctxs) {
        ...
        // Allocate ram_buffer in named shared memory so child processes can
        // read it via OpenFileMapping (zero-copy).  cv::Mat wraps the shared
        // memory pointer — it does NOT own the memory.
        ...
        if (!hMap) {
            logException("FATAL", "shm", "CreateFileMapping failed for " + shm_name + " error " + to_string(GetLastError()));
            return 1;
        }
        uint8_t* shm_base = (uint8_t*)MapViewOfFile(hMap, FILE_MAP_WRITE, 0, 0, total_bytes);
        if (!shm_base) {
            logException("FATAL", "shm", "MapViewOfFile failed for " + shm_name + " error " + to_string(GetLastError()));
            CloseHandle(hMap);
            return 1;
        }
        // Zero-initialize the shared memory (replaces cv::Mat::zeros)
        memset(shm_base, 0, total_bytes);

        for (int k = 0; k < total_record_frames; ++k) {
            ctx->ram_buffer[k] = cv::Mat(cam_h, cam_w, CV_8UC1,
                                         shm_base + (size_t)k * (size_t)cam_h * (size_t)cam_w);
        }
        ctx->shm_handle = hMap;
        ctx->shm_base   = shm_base;
    }
    cout << "[System] Shared memory allocated. Pipeline ready.\n" << endl;
    ...
    while (global_running) {
        ...
        if (is_recording && !is_dumping) {
            ...
            if (all_done) {
                ...
                // Step 0: Pre-create HDF5 files + datasets for all cameras (serial, ~0.3s total)
                for (auto& ctx : cam_ctxs) {
                    ctx->dump_start_time = chrono::steady_clock::now();
                    stringstream pss; pss << ctx->hdf5_dir << "/" << setw(4) << setfill('0') << g_chunk_idx << ".h5";
                    bool exists = fs::exists(pss.str());
                    if (!exists) {
                        try {
                            ...
                            logException("ERROR", "hdf5:precreate",
                                "Failed to create HDF5 file for cam " + to_string(ctx->index) + ": " + e.getCDetailMsg());
                        }
                    }
                }

                // Locate child executable (same directory as parent)
                ...
                // Step 1: Create Job Object + launch child processes
                ...
                for (size_t i = 0; i < cam_ctxs.size(); ++i) {
                    ...
                    stringstream args;
                    // Fake argv[0] — Windows C runtime uses the first space-delimited
                    // token as argv[0]; without this, all args shift by one position.
                    ...
                    // Store cmd_line in a local string — CreateProcessA may modify
                    // lpCommandLine in-place (inserts \0 during parsing).
                    // Passing args.str().c_str() via (LPSTR) would cause UB.
                    string cmd_line = args.str();
                    if (CreateProcessA(child_exe.c_str(), &cmd_line[0],
                                       NULL, NULL, FALSE, 0, NULL, NULL, &si, &pi)) {
                        CloseHandle(pi.hThread);
                        if (hJob) AssignProcessToJobObject(hJob, pi.hProcess);
                        procs[i] = pi;
                    } else {
                        logException("ERROR", "hdf5:proc",
                            "CreateProcess failed for cam " + to_string(i) + " error " + to_string(GetLastError()));
                        procs[i].hProcess = NULL;
                    }
                }
                // NOTE: Do NOT close hJob here — JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE
                // would immediately terminate all children.

                // Show dump status
                {
                    cv::Mat loading = cv::Mat::zeros(400, 600, CV_8UC3);
                    cv::putText(loading, "DUMPING RAM TO HDF5... (" + to_string(cam_ctxs.size()) + " processes)", cv::Point(50, 200), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 255, 255), 2);
                ...
                for (auto& p : procs) if (p.hProcess) handles.push_back(p.hProcess);
                if (!handles.empty())
                ...
                bool all_ok = true;
                for (auto& p : procs) {
                    if (!p.hProcess) { all_ok = false; continue; }
                    DWORD ec;
                    if (GetExitCodeProcess(p.hProcess, &ec)) {
                        if (ec != 0) {
                            all_ok = false;
                            logException("ERROR", "hdf5:cam", "child process exit code " + to_string(ec));
                        }
                    }
                    CloseHandle(p.hProcess);
                }
                if (hJob) CloseHandle(hJob);  // Safe now — all children have exited

                double dump_duration = chrono::duration<double>(chrono::steady_clock::now() - dump_start_time).count();

                // record per-camera dump_end_time (approximate — all finish at ~same time)
                for (auto& ctx : cam_ctxs)
                    ctx->dump_end_time = chrono::steady_clock::now();
