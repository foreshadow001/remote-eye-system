...
struct CameraContext {
    ...
    // DMA: store Pylon GrabResultPtr directly, no CPU memcpy
    vector<Pylon::CBaslerUniversalGrabResultPtr> dma_frames;
...
void dumpToDiskWorker(shared_ptr<CameraContext> ctx, int write_delay_ms, atomic<int>& finished_cams) {
    ...
    for (int i = 0; i < frames_to_dump; ++i) {
        ...
        auto& ptr = ctx->dma_frames[i];

        string filename = ctx->temp_dir + "/" + to_string(meta.blockID) + ".raw";

        std::ofstream out_raw(filename, std::ios::binary);
        if (ptr && out_raw) {
            size_t sz = ptr->GetWidth() * ptr->GetHeight();
            out_raw.write(reinterpret_cast<const char*>(ptr->GetBuffer()), sz);
            if (!out_raw.good()) {
            ...
            logException("ERROR", "dump:cam" + to_string(ctx->index) + ":" + ctx->id,
                         "Null DMA frame at index " + to_string(i));
            break;
        } else {
            logException("ERROR", "dump:cam" + to_string(ctx->index) + ":" + ctx->id,
        ...
        if (ctx->log_stream.is_open()) {
            int w = ptr ? ptr->GetWidth() : 0, h = ptr ? ptr->GetHeight() : 0;
...
int main() {
    ...
    cout << "[System] Reserving DMA frame slots for " << total_record_frames << " frames..." << endl;
    for (auto& ctx : cam_ctxs) {
        ctx->total_record_frames = total_record_frames;
        ctx->dma_frames.resize(total_record_frames);  // GrabResultPtr vector, no memory allocation
        ctx->meta_buffer.resize(total_record_frames);
    }
    cout << "[System] DMA slots reserved. Pipeline ready.\n" << endl;
