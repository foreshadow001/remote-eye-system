...
void captureWorker(shared_ptr<CameraContext> ctx,double fps,double gain,double gamma,double exp,double me){
    ...
    // [Retrieve] Pylon Viewer 式取流: GrabLoop_ProvidedByUser + RetrieveResult
    if(!ctx->cam.startUserLoop())return;
    while(ctx->running){
        Pylon::CBaslerUniversalGrabResultPtr p;
        if(!ctx->cam.retrieveResult(p,1000)) continue;  // 超时重试
        if(!p->GrabSucceeded()) continue;
        FrameMeta m;
        m.blockID=p->GetBlockID(); m.timestamp=p->GetTimeStamp();
        m.sys_time_ms=(double)cv::getTickCount()/cv::getTickFrequency()*1000.0;
        // [DBG] 坏帧检测 (RetrieveResult 路径)
        {size_t w=p->GetWidth(),h=p->GetHeight(),ps=p->GetPayloadSize();
            if(ps!=w*h)
                cerr<<"[BadFrame] cam "<<ctx->sn<<" blk="<<m.blockID
                    <<" size="<<w<<"x"<<h<<" payload="<<ps<<endl;}
...
// ================== main ==================
int main(){
    cout<<"=== Calibration Image Capture (TCP, RetrieveResult path) ==="<<endl;
#ifdef _WIN32
    ...
    // ---- UI ----
    cv::namedWindow("Calib Retrieve",cv::WINDOW_NORMAL);cv::resizeWindow("Calib Retrieve",g_win_w,g_win_h);updateLayout();cv::setMouseCallback("Calib Retrieve",onMouse);
    auto uii=chrono::milliseconds((int)(1000.0/uif));auto lui=chrono::steady_clock::now()-uii;
    ...
    while(global_running){
        ...
        cv::imshow("Calib Retrieve",cv);lui=now;}
