...
void captureWorker(shared_ptr<CameraContext> ctx,double fps,double gain,double gamma,double exp,double me){
    ...
    // [DBG] 帧级诊断: 在回调内(缓冲保证有效的窗口)检查 + 落盘
    static const string dbg_dir="D:/calib_debug";
    ...
    ctx->cam.setFrameCallback([ctx](const Pylon::CBaslerUniversalGrabResultPtr& p,FrameMeta m){
        ...
        // 每台相机: 前3帧落盘作参考 + 前20帧打印指针地址(复用检测)
        static thread_local int frame_cnt=0;
        static thread_local void* last_ptr=nullptr;
        frame_cnt++;
        void* bp=p->GetBuffer();
        if(frame_cnt<=3){
            stringstream fn;fn<<dbg_dir<<"/good_"<<ctx->sn<<"_blk"<<m.blockID<<"_"<<w<<"x"<<h<<".raw";
            ofstream of(fn.str(),ios::binary);of.write((const char*)bp,ps);of.close();
            cout<<"[DBG] cam "<<ctx->sn<<" blk="<<m.blockID<<" frame#"<<frame_cnt
                <<" ptr="<<bp<<" size="<<w<<"x"<<h<<" payload="<<ps<<endl;
        } else if(frame_cnt<=20&&frame_cnt%5==0){
            cout<<"[DBG] cam "<<ctx->sn<<" blk="<<m.blockID<<" frame#"<<frame_cnt
                <<" ptr="<<bp<<" size="<<w<<"x"<<h<<" payload="<<ps<<endl;
        }
        // 坏帧: 打印 + 原始落盘 (前20个)
        if(!gs||ps!=w*h){
            static thread_local int bad_cnt=0;
            if(bad_cnt<20){
                bad_cnt++;
                stringstream fn;fn<<dbg_dir<<"/bad_"<<ctx->sn<<"_blk"<<m.blockID<<"_"<<w<<"x"<<h<<"_p"<<ps<<".raw";
                ofstream of(fn.str(),ios::binary);of.write((const char*)bp,ps);of.close();
            }
            cerr<<"[BadFrame] cam "<<ctx->sn<<" blk="<<m.blockID<<" frame#"<<frame_cnt
                <<" grab="<<(gs?"OK":"FAIL")
                <<" size="<<w<<"x"<<h<<" payload="<<ps
                <<" ptr="<<bp
                <<(ps==w*h?" (match)":" (MISMATCH)")<<endl;
        }
        cv::Mat tmp((int)h,(int)w,CV_8UC1,bp);
