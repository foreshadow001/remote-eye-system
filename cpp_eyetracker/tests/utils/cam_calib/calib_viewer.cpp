...
// ================== Camera (与 Pylon Viewer 完全平行: 纯 Open, 不设置任何相机/流参数) ==================
struct CameraContext {string sn; bool is_mono=true; thread capture_thread,copy_thread; atomic<bool> running{true};
    cv::Mat latest_frame; FrameMeta latest_meta; mutex frame_mtx;
    queue<pair<cv::Mat,FrameMeta>> copy_queue; mutex copy_mtx; condition_variable copy_cv;
    ...
    explicit CameraContext(string s):sn(s){}};
...
// 事件处理器 — 与官方 Grab_UsingGrabLoopThread 样本一致
struct ViewerImageHandler : public Pylon::CImageEventHandler {
    shared_ptr<CameraContext> ctx;
    explicit ViewerImageHandler(shared_ptr<CameraContext> c):ctx(c){}
    void OnImageGrabbed(Pylon::CInstantCamera&, const Pylon::CGrabResultPtr& p) override {
        if(!p->GrabSucceeded()) return;
        // 首帧: 从实际像素格式推断 mono/color (不预设)
        if(!ctx->has_streamed.load()){
            uint64_t pt=p->GetPixelType();
            ctx->is_mono = (pt==Pylon::PixelType_Mono8);
            cout<<"[Viewer] cam "<<ctx->sn<<" pixelType="<<hex<<pt<<dec
                <<(ctx->is_mono?" (MONO)":" (COLOR)")<<endl;
        }
        FrameMeta m; m.blockID=p->GetBlockID(); m.timestamp=p->GetTimeStamp();
        m.sys_time_ms=(double)cv::getTickCount()/cv::getTickFrequency()*1000.0;
        cv::Mat tmp((int)p->GetHeight(),(int)p->GetWidth(),CV_8UC1,p->GetBuffer());
...
void captureWorker(shared_ptr<CameraContext> ctx){
    try {
        ...
        info.SetSerialNumber(Pylon::String_t(ctx->sn.c_str()));
        ...
        cerr<<"[Error] cam "<<ctx->sn<<": "<<e.GetDescription()<<endl;
    }
}
...
int main(){
    ...
    for(auto& ctx:cam_ctxs){ctx->running=true;ctx->copy_thread=thread(copyWorker,ctx);ctx->capture_thread=thread(captureWorker,ctx);}
