...
int main() {
    ...
    if (!calib_data_left.empty()) {
        ...
        auto calibration_result_left = calibration_left.calibrate(gazetracker_left, params_left, calib_data_left, init_left, bounds);
        ...
        std::ofstream fout_left(output_dir + "/calib_inference_result_left_single.txt");
        if (fout_left) {
            ...
            fout_left.close();
            std::cout << "\nLeft eye inference result saved to " << output_dir << "/calib_inference_result_left_single.txt\n";
        }
    } else {
    ...
    if (!calib_data_right.empty()) {
        ...
        auto calibration_result_right = calibration_right.calibrate(gazetracker_right, params_right, calib_data_right, init_right, bounds);
        ...
        // 输出右眼推理结果
        std::ofstream fout_right(output_dir + "/calib_inference_result_right_single.txt");
        if (fout_right) {
            ...
            fout_right.close();
            std::cout << "\nRight eye inference result saved to " << output_dir << "/calib_inference_result_right_single.txt\n";
        }
    } else {
