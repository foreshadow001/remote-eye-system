...
int main() {
    ...
    // 获取每个文件夹采样的图片数量，校准程序中默认采用 50 张（对应原程序的 start_frame + 50 逻辑）
    ...
    // === 4. 输出推理结果 ===
    std::ofstream fout(output_dir + "/calib_inference_result_left.txt");
    if (!fout) {
        std::cerr << "[ERROR] Cannot open file: " << output_dir << "/calib_inference_result_left.txt\n";
        return -1;
    }
    // 表头
    ...
    fout.close();
    std::cout << "\ninference result saved to " << output_dir << "/calib_inference_result_left.txt\n";

    std::ofstream fout_right(output_dir + "/calib_inference_result_right.txt");
    if (!fout_right) {
        std::cerr << "[ERROR] Cannot open file: " << output_dir << "/calib_inference_result_right.txt\n";
        return -1;
    }
    ...
    fout_right.close();
    std::cout << "\ninference result saved to " << output_dir << "/calib_inference_result_right.txt\n";
