 ...
 * @param level: 阶数 (1: 1点, 2: 5点菱形, 3: 13点菱形, 4: 25点菱形)
 * @param k: 两端分别剔除的极值个数
 */
double calculateDiamondAverage(const cv::Mat& img, double cx, double cy, int level, int k)
{
    ...
    pixels.reserve((max_dist + 1) * (max_dist + 1) * 2); // 预分配空间
